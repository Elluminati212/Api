...
MONGO_URI = "mongodb+srv://vasu:elluminati@cabe.oyyivud.mongodb.net/"

# Create a client to connect to MongoDB
try:
    client = MongoClient("mongodb+srv://vasu:elluminati@cabe.oyyivud.mongodb.net/")
    db = client.get_database("CabE")  #  database name
...
client = MongoClient("mongodb+srv://vasu:elluminati@cabe.oyyivud.mongodb.net/")
db = client["CabE"]
trip_histories_collection = db["trip_histories"]
...
def fetch_chunk_by_date_range(start_date, end_date, skip, limit):
    # Re-initialize MongoDB client for each thread
    client = MongoClient("mongodb+srv://vasu:elluminati@cabe.oyyivud.mongodb.net/")
    db = client["CabE"]
    collection = db["trip_histories"]
    query = {
    ...
    cursor = collection.find(query, fields_to_fetch).skip(skip).limit(limit)
    return list(cursor)

# Function to process data in parallel
def fetch_data_by_month(start_date, end_date, chunk_size):
    total_docs = trip_histories_collection.count_documents({
        "provider_trip_start_time": {
            "$gte": start_date,
            "$lt": end_date
        }
    ...
    with ThreadPoolExecutor() as executor:
        ...
        for i in range(0, total_docs, chunk_size):
            futures.append(executor.submit(fetch_chunk_by_date_range, start_date, end_date, i, chunk_size))
...
def fetch_data_for_all_months(start_date, end_date, chunk_size):
    ...
    while current_date < end_date:
        ...
        monthly_data = fetch_data_by_month(first_day_of_month, next_month, chunk_size)
...
end_date = datetime(2024, 11, 30)  # Adjust as needed
chunk_size = 1000  # Number of documents per thread

# Fetch data for all months
all_data = fetch_data_for_all_months(start_date, end_date, chunk_size)
...
def get_api_credentials():
    ...
    return {
        "data": {
            "serviceConfig": {
                "dbUrl": "mongodb+srv://vasu:elluminati@cabe.oyyivud.mongodb.net/",
                "dbName": "CabE",
                "collection": "trip_histories"
            }
        }
    }
...
if __name__ == "__main__":
    ...
    end_date = datetime(2024, 12, 30)  # Adjust as needed
    chunk_size = 1000  # Number of documents per thread
